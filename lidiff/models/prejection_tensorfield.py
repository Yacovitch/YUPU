...
class RealisticProjection(object):
    ...
    def get_img(self, tensor_field, is_cls=False):
        """
        Get images from a TensorField.

        Args:
            tensor_field (ME.TensorField): TensorField containing coordinates and features.
            is_cls (bool): Whether to use classification-style image projection.

        Returns:
            img (torch.tensor): Projected images of size [B * num_views, resolution, resolution].
            is_seen (torch.tensor): Boolean tensor indicating if each point is visible in each view.
            point_loc_in_img (torch.tensor): Point locations in the image.
        """
        # Extract coordinates from the TensorField
        coordinates = tensor_field.coordinates  # Shape: [N, D] (continuous or discrete coordinates)
        print(tensor_field.coordinates[:5])

        # Transform the points (rotate and translate)
        #b, _ = coordinates.shape
        b = 1
        v = self.translation.shape[0]
        self._points = self.point_transform(
            points=torch.repeat_interleave(coordinates, v, dim=0),
        ...
        # Convert transformed coordinates into a grid
        grid, xx, yy, zz_int, zz, nnbatch = points2grid(
            points=self._points, params=self.params, device=self.device
        )
        ...
        # Determine visibility and point locations
