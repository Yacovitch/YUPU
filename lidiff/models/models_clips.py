...
class DiffusionPoints(LightningModule):
    def __init__(self, hparams:dict, data_module: LightningDataModule = None, config_path=None):
        ...
        self.img_extractor = Extractor_img(device = device, n_points = self.hparams['data']['num_points']//4, params_override=proj_params)
    ...
    def forward(self, x_full, x_full_sparse, x_part, t, pcd_part, mean, std):
        ...
        if torch.all(pcd_part == 0):
            ...
            img_condition_emb, is_seen, point_loc = vanilla_upprojection(
                    img_feat, is_seen, point_loc_in_img, img_size=(224, 224), n_points=self.hparams['data']['num_points']//4, vweights=None
                )
                
            img_feat = self.points_to_tensor_with_features(pcd_part, img_condition_emb, mean, std)
            img_feat = self.partial_enc_img(img_feat)
            
            # Concatenate feature matrices with coordinate alignment to avoid row mismatches
            matched_img_feats = self.model.match_part_to_full(part_feat, img_feat)
            concatenated_features = torch.cat([part_feat.F, matched_img_feats], dim=1)
