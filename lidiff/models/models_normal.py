...
class DiffusionPoints(LightningModule):
    ...
    def classfree_forward(self, x_t, x_cond, x_uncond, normals, t):
        x_t_sparse = x_t.sparse()
        x_cond = self.forward(x_t, x_t_sparse, x_cond, normals, t)            
        x_uncond = self.forward(x_t, x_t_sparse, x_uncond, normals, t)
    ...
    def p_sample_loop(self, x_init, x_t, x_cond, x_uncond, gt_pts, x_mean, x_std, normals):
        ...
        for t in tqdm(range(len(self.dpm_scheduler.timesteps))):
            ...
            noise_t = self.classfree_forward(x_t, x_cond, x_uncond, normals, t)
    ...
    def forward(self, x_full, x_full_sparse, x_part, normals, t):
        #print('forward')
        ...
        concatenated_features = torch.cat([part_feat.F, matched_normal_feats], dim=1)
    ...
    def training_step(self, batch:dict, batch_idx):
        ...
        denoise_t = self.forward(x_full, x_full.sparse(), x_part, x_normals, t)
    ...
    def validation_step(self, batch:dict, batch_idx):
        ...
        with torch.no_grad():
            ...
            x_gen_eval = self.p_sample_loop(x_init, x_full, x_part, x_uncond, gt_pts, batch['mean'], batch['std'], x_normals)
    ...
    def test_step(self, batch:dict, batch_idx):
        ...
        with torch.no_grad():
            ...
            x_gen_eval = self.p_sample_loop(x_init, x_full, x_part, x_uncond, gt_pts, torch.zeros_like(batch['pcd_part']), batch['mean'], batch['std'], x_normals)
            x_gen_eval = x_gen_eval.F.reshape((gt_pts.shape[0],-1,3))
