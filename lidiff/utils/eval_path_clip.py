...
@click.command()
@click.option('--path', '-p', type=str, default='/nas2/jacob/LiDiff/lidiff/results/reverse_diffusion_clip_0/diff', help='path to the scan sequence')
...
@click.option('--diff', '-d', type=str, default='/nas2/jacob/LiDiff_HPC/experiments/prob10_5p0reg/default/version_26/checkpoints/prob10_5p0reg_epoch=19.ckpt', help='run diffusion pipeline')
...
@click.option('--model_type',
              '-m',
              type=click.Choice(['clip', 'resnet']),
              help='Type of model to use (clip or resnet)',
              default='resnet')
def main(path, voxel_size, max_range, denoising_steps, cond_weight, diff, refine, save_reverse_diffusion, model_type): 
    #diff_completion = DiffCompletion(diff, refine, denoising_steps, cond_weight, model_type)
