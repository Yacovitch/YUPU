...
class SimplePointCloud:
    ...
    def select_by_index(self, indices: List[int], invert: bool = False) -> 'SimplePointCloud':
        mask = np.zeros(len(self.points), dtype=bool)
        mask[indices] = True
        if invert:
            mask = ~mask
            
        new_pcd = SimplePointCloud()
        new_pcd.points = self.points[mask]
        if self.colors is not None:
            new_pcd.colors = self.colors[mask]
        return new_pcd

def clusterize_pcd(points, ground):
    pcd = SimplePointCloud()
    pcd.set_points(points[:, :3])

    # instead of ransac use patchwork
    inliers = list(np.where(ground == 9)[0])

    pcd_ = pcd.select_by_index(inliers, invert=True)
    labels_ = np.expand_dims(clusters_hdbscan(pcd_.points), axis=-1)
...
def visualize_pcd_clusters(points, labels):
    ...
    pcd.set_colors(colors[:,:3])
    
    # Save the point cloud to a PLY file instead of visualizing
    save_ply(pcd.points, pcd.colors, "visualization.ply")
