...
import lidiff.models.models_clip_only as models
import lidiff.models.models_resnet as models_resnet
...
@click.option('--model_type',
              ...
              default='clip')
def main(config, weights, checkpoint, test, model_type):
    set_deterministic()
    ...
    if weights is None:
        if model_type == 'clip':
            ...
            model = models_resnet.DiffusionPoints(cfg, config_path=config)
    else:
        if test:
        ...
        if model_type == 'clip':
            ...
            model = models_resnet.DiffusionPoints.load_from_checkpoint(weights, hparams=cfg, config_path=config)
        print(model.hparams)
