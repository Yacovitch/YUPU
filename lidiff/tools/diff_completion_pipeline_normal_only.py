...
class CrossModalMixer(torch.nn.Module):
    def __init__(self, embed_dim=256, num_heads=8, dropout=0.0):
        ...
        self.attn = torch.nn.MultiheadAttention(embed_dim=embed_dim, num_heads=num_heads, dropout=dropout, batch_first=True)
    ...
    def forward(self, x: ME.SparseTensor) -> ME.SparseTensor:
        feats = x.F  # [N, 512] -> two 256D modalities
        part_feats, img_feats = torch.split(feats, self.embed_dim, dim=1)
        tokens = torch.stack([part_feats, img_feats], dim=1)  # [N, 2, 256]
        ...
        fused = torch.cat([tokens[:, 0, :], tokens[:, 1, :]], dim=1)  # [N, 512]
        fused = self.out_proj(fused)  # [N, 512]
...
class DiffCompletion(LightningModule):
    def __init__(self, diff_path, refine_path, denoising_steps, cond_weight):
        ...
        #self.modality_mixer = CrossModalMixer(embed_dim=256, num_heads=8, dropout=0.0)
        
        
        self.model = minknet.MinkUNetDiff_abb_study(in_channels=3, out_channels=self.hparams['model']['out_dim']).cuda()
    ...
    def reset_partial_pcd(self, x_part, x_uncond, normals):
        new_x_part = self.points_to_tensor(x_part.F.reshape(1,-1,3).detach())
        new_x_uncond = self.points_to_tensor(torch.zeros_like(x_part.F.reshape(1,-1,3)))
        normals_feats = normals.F.reshape(1, -1, normals.F.shape[1]).detach()
        new_normals = self.points_to_tensor_with_features(
            x_part.F.reshape(1,-1,3), normals_feats)
    ...
    def forward(self, x_full, x_full_sparse, x_part, t, pcd_part, normals):
        ...
        with torch.no_grad():
            ...
            if torch.all(pcd_part == 0):
                concatenated_features = part_feat.F
            else:
                normal_feat = self.partial_enc_normal(normals)
                concatenated_features = self.model.match_part_to_full(part_feat, normal_feat)
    ...
    def completion_loop(self, x_init, x_t, x_cond, x_uncond, save_reverse_diffusion, x_part_batch, normals):
        ...
        for t in tqdm.tqdm(range(len(self.dpm_scheduler.timesteps))):
            ...
            x_cond, x_uncond, normals = self.reset_partial_pcd(x_cond, x_uncond, normals)
            torch.cuda.empty_cache()
        if save_reverse_diffusion:
...
@click.command()
@click.option('--diff', '-d', type=str, default='/nas2/jacob/LiDiff_HPC/experiments/yupu_normal_only/default/version_5/checkpoints/yupu_normal_only_epoch=19.ckpt', help='path to the scan sequence')
