...
class DiffCompletion(LightningModule):
    ...
    def preprocess_scan(self, scan):
        ...
        up_ratio = int(self.hparams['data'].get('upsample_ratio', 4))
        target = int(self.hparams['data']['num_points'] / up_ratio)
        n_pts = int(scan.shape[0])
        print('scan sizes: ', target, n_pts)
        if target <= 0:
            target = max(1, n_pts)
        if target < n_pts:
...
@click.command()
@click.option('--diff', '-d', type=str, default='/nas2/jacob/LiDiff_HPC/experiments/yupu_lidiff/default/version_0/checkpoints/yupu_lidiff_epoch=19.ckpt', help='path to the scan sequence')
