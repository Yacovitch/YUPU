...
class DiffCompletion(LightningModule):
    ...
    def preprocess_scan(self, scan):
        ...
        scan = scan[(dist < self.hparams['data']['max_range'])][:,:3]
        n_point = int(scan.shape[0])
        #scan = scan.repeat(concat_part, 0)
        # use farthest point sampling
        if n_point > np.ceil(self.hparams['data']['num_points'] / 10):
            pcd_scan = o3d.geometry.PointCloud()
            pcd_scan.points = o3d.utility.Vector3dVector(scan)
            pcd_scan = pcd_scan.farthest_point_down_sample(np.ceil(self.hparams['data']['num_points'] / 10))
            scan = pcd_scan.points
        else:
            concat = np.ceil(self.hparams['data']['num_points'] / 10 / scan.shape[0])
            scan = scan[torch.randperm(scan.shape[0])]
            scan = scan.repeat(concat, 0)
        #pcd_scan = pcd_scan.voxel_down_sample(voxel_size=0.2)
        
        #scan = scan[torch.randperm(scan.shape[0])]
        #scan = scan.repeat(concat, 0)
        
        #scan = scan[:int(self.hparams['data']['num_points'] / 10)]
    ...
    def preprocess_scan_grid(self, scan):
        dist = np.sqrt(np.sum((scan)**2, -1))
        scan = scan[(dist < self.hparams['data']['max_range'])][:,:3]
    ...
    def complete_scan(self, scan, save_reverse_diffusion):
        scan = self.preprocess_scan_grid(scan)
        ...
        if save_reverse_diffusion:
            completed_scan, reverse_steps = self.completion_loop(scan, x_full, x_cond, x_uncond, save_reverse_diffusion)
        else:
            completed_scan = self.completion_loop(scan, x_full, x_cond, x_uncond, save_reverse_diffusion)
    ...
    def completion_loop(self, x_init, x_t, x_cond, x_uncond, save_reverse_diffusion):
        self.scheduler_to_cuda()
        saved_steps = []
...
@click.command()
@click.option('--diff', '-d', type=str, default='/nas2/jacob/LiDiff/lidiff/experiments/sensat_random_radius_50/default/version_13/checkpoints/sensat_random_radius_50_epoch=19.ckpt', help='path to the scan sequence')
...
def main(diff, refine, denoising_steps, cond_weight, save_reverse_diffusion):
    ...
    data_list = [k for k in os.listdir(path) if 'sub_0' in k]
